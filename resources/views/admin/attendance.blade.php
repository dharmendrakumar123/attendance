...
@section('content')
		...
		@endif
                                    ...
                                    <tr>
										...
										<?php
											...
											for ($i = 1; $i <= $getlastday; $i++) {
												//$list = $currentmonth . '-' . str_pad($i, 2, '0', STR_PAD_LEFT);
                                    ...
                                    <tr> 
										<td>
                                            <h2 class="table-avatar">
                                                <a class="avatar avatar-xs" href="{{route('allemployeeprofile',$PunchModels->uid)}}"><img alt="" src="images/{{$PunchModels->profile_pic}} "></a>
                                                <a href="{{route('allemployeeprofile',$PunchModels->uid)}}">{{$PunchModels->name}} {{$PunchModels->last_name}}</a>
                                            </h2>
                                        </td>
											<?php
													...
													if(!empty($employeeattendance[$i])){
													    if($employeeattendance[$i]["uid"] == $PunchModels->uid){	
														    if($employeeattendance[$i]["time_in"]!=="00:00:00"){
																...
																if($totalhours<=6){
																	echo '<td><a href="#" class="edit_attendance" id="'.$employeeattendance[$i]["id"].'">0.5</a></td>';
																}elseif(($totalhours>6)&&($totalhours<8)){
																	echo '<td><a href="#" class="edit_attendance" id="'.$employeeattendance[$i]["id"].'">0.75</a></td>';
																}else{
																	echo '<td><a href="#" class="edit_attendance" id="'.$employeeattendance[$i]["id"].'">1</a></td>';
																}
															}else{
																echo '<td><a href="#" class="edit_attendance" id="'.$employeeattendance[$i]["id"].'"><span class="first-off">0</span></a></td>';	
															} 
														}
													} 
												}						
			...
			<div class="modal custom-modal fade" id="admin_edit_attendance" role="dialog">
                <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
							<form action="{{route('admin_update_attendance')}}" method="post">
								@csrf
